...
BEGIN
	...
	BEGIN
		...
		--nodes
		DECLARE @tempNodeIds TABLE(NodeId INT, CatalogId INT)
		INSERT INTO @tempNodeIds
		SELECT n.CatalogNodeId, n.CatalogId
		FROM CatalogNode n
		...
		UPDATE d SET 
			...
			MasterLanguageName = c.LanguageName
		FROM ecfVersion d
		INNER JOIN @tempNodeIds n ON (n.NodeId = d.ObjectId AND d.ObjectTypeId = 1)
		INNER JOIN @Catalogs c ON c.ObjectId = n.CatalogId
		LEFT JOIN @VersionCatalogs l ON l.WorkId = c.WorkId

		--entries
		DECLARE @tempEntrieIds TABLE(EntryId INT, CatalogId INT)
		INSERT INTO @tempEntrieIds
		SELECT e.CatalogEntryId, e.CatalogId
		FROM CatalogEntry e
		...
		UPDATE d SET 
			...
			MasterLanguageName = c.LanguageName
		FROM ecfVersion d
		INNER JOIN @tempEntrieIds e ON (e.EntryId = d.ObjectId AND d.ObjectTypeId = 0)
		INNER JOIN @Catalogs c ON c.ObjectId = e.CatalogId
		LEFT JOIN @VersionCatalogs l ON l.WorkId = c.WorkId
	END
END
